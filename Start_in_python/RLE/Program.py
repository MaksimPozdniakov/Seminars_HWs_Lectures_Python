...
with open('compression_algorithm.txt', 'w', encoding='UTF-8') as my_str:
    my_str.write('Строка после сжатия: ')
    my_str.write(result)

with open('compression_algorithm.txt', 'r', encoding='UTF-8') as my_str:
    my_str1 = my_str.readline()
my_str1 = my_str1.replace('Строка после сжатия: ', '')

new_result = decryption_algorithm(my_str1)

with open('decryption_algorithm.txt', 'w', encoding='UTF-8') as new_str:
